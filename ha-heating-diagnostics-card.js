...
class HAHeatingDiagnosticsCard extends HTMLElement {
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `<style>
      :host{display:block;--good:#54d29b;--learn:#64a9ff;--warn:#ffc45c;--bad:#ff667a;--heat:#ff8a3d;--edge:rgba(255,255,255,.11)}*{box-sizing:border-box}
      ...
      .flow{position:relative;height:70px;margin-bottom:16px;border:1px solid var(--edge);border-radius:18px;background:linear-gradient(90deg,rgba(255,138,61,.09),rgba(100,169,255,.05));overflow:hidden}.flow-line{position:absolute;left:7%;right:7%;top:50%;height:3px;border-radius:9px;background:linear-gradient(90deg,var(--heat),var(--warn),var(--learn));box-shadow:0 0 18px rgba(255,138,61,.3)}.flow-line:after{content:"";position:absolute;width:60px;inset:-2px auto -2px -60px;background:linear-gradient(90deg,transparent,#fff,transparent);animation:flow 3s linear infinite}.flow-node{position:absolute;top:50%;transform:translateY(-50%);display:flex;align-items:center;gap:10px;padding:8px 11px;border:1px solid var(--edge);border-radius:12px;background:var(--ha-card-background);box-shadow:0 8px 22px rgba(0,0,0,.2)}.flow-node ha-icon{flex:0 0 25px;width:25px;height:25px;--mdc-icon-size:25px}.flow-node>div{min-width:0}.flow-node strong{display:block;font-size:12px;white-space:nowrap}.flow-node small{display:block;margin-bottom:2px;color:var(--secondary-text-color);font-size:9px;text-transform:uppercase;letter-spacing:.08em}.flow-node.source{left:3%;color:var(--heat)}.flow-node.model{left:50%;transform:translate(-50%,-50%);color:var(--learn)}.flow-node.rooms{right:3%;color:var(--good)}
      .room-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.room{--tone:var(--good);position:relative;display:flex;flex-direction:column;overflow:hidden;min-width:0;padding:16px;border:1px solid color-mix(in srgb,var(--tone) 24%,var(--edge));border-radius:19px;background:linear-gradient(145deg,color-mix(in srgb,var(--tone) 5%,transparent),rgba(0,0,0,.035));box-shadow:0 8px 25px rgba(0,0,0,.08);transition:transform .2s ease,box-shadow .2s ease}.room.learning{--tone:var(--learn)}.room.warn{--tone:var(--warn)}.room.problem{--tone:var(--bad)}.room.info{--tone:var(--learn)}.room:hover{transform:translateY(-2px);box-shadow:0 13px 30px rgba(0,0,0,.15),0 0 0 1px color-mix(in srgb,var(--tone) 18%,transparent)}.room-ambient{position:absolute;right:-60px;top:-80px;width:180px;height:180px;border-radius:50%;background:var(--tone);opacity:.08;filter:blur(28px);pointer-events:none}.room.problem .room-ambient,.room.active .room-ambient{animation:breathe 2.4s ease-in-out infinite}
      ...
      <div class="room-grid">${rooms.map((room,index)=>this._room(room,index)).join("")}</div>
    </div></ha-card>`;
